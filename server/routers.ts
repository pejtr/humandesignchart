...
Vytvoř osobní denní tranzitový výklad pro tuto osobu.`;
    ...
    // Personalized transit reading by chartData (for unsaved/current charts)
    personalizedByData: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        const isEn = input.locale === 'en';
        const chartData = input.chartData as import("../shared/types").HumanDesignChartData;
        const { calculatePlanetaryPositions, dateToJD } = await import("./humandesign/ephemeris");
        const { GATE_WHEEL, PLANET_NAMES } = await import("./humandesign/constants");
        const now = new Date();
        const jd = dateToJD(now);
        ...
        for (const [planet, lon] of Object.entries(positions)) {
          const lonNum = typeof lon === "number" ? lon : 0;
          const gateIdx = Math.floor((lonNum % 360) / (360 / 64));
          const gate = (GATE_WHEEL as unknown as number[])[gateIdx] || 1;
          const line = Math.floor(((lonNum % (360 / 64)) / (360 / 64)) * 6) + 1;
          const name = (PLANET_NAMES as unknown as Record<string, string>)[planet] || planet;
          transitGates.push({ planet: name, gate, line });
        }
        const natalGates = new Set<number>((chartData.activatedGates as number[]) || []);
        const transitGateNums = new Set(transitGates.map(t => t.gate));
        const CHANNELS: [number, number][] = [[1,8],[2,14],[3,60],[4,63],[5,15],[6,59],[7,31],[8,1],[9,52],[10,20],[11,56],[12,22],[13,33],[14,2],[15,5],[16,48],[17,62],[18,58],[19,49],[20,10],[21,45],[22,12],[23,43],[24,61],[25,51],[26,44],[27,50],[28,38],[29,46],[30,41],[31,7],[32,54],[33,13],[34,20],[35,36],[36,35],[37,40],[38,28],[39,55],[40,37],[41,30],[42,53],[43,23],[44,26],[45,21],[46,29],[47,64],[48,16],[49,19],[50,27],[51,25],[52,9],[53,42],[54,32],[55,39],[56,11],[57,34],[58,18],[59,6],[60,3],[61,24],[62,17],[63,4],[64,47]];
        const activatedChannels: Array<{ gate1: number; gate2: number; via: string }> = [];
        for (const [g1, g2] of CHANNELS) {
          const tHasG1 = transitGateNums.has(g1), tHasG2 = transitGateNums.has(g2);
          const nHasG1 = natalGates.has(g1), nHasG2 = natalGates.has(g2);
          if ((tHasG1 && nHasG2) || (tHasG2 && nHasG1)) activatedChannels.push({ gate1: g1, gate2: g2, via: tHasG1 ? `Tranzit brána ${g1} + natalální brána ${g2}` : `Tranzit brána ${g2} + natalální brána ${g1}` });
        }
        const reinforcedGates = transitGates.filter(t => natalGates.has(t.gate));
        const PLANET_SYM: Record<string, string> = { Sun: "☉", Moon: "☽", Mercury: "☿", Venus: "♀", Mars: "♂", Jupiter: "♃", Saturn: "♄", Uranus: "⛢", Neptune: "♆", Pluto: "♇", "North Node": "☊", "South Node": "☋" };
        const transitSummary = transitGates.map(t => `${PLANET_SYM[t.planet] || t.planet} ${t.planet}: ${isEn ? 'Gate' : 'Brána'} ${t.gate}.${t.line}`).join(", ");
        ...
        const rawContent = response.choices?.[0]?.message?.content;
        const interpretation = typeof rawContent === "string" ? rawContent : (isEn ? "Failed to generate reading." : "Nepodařilo se vygenerovat výklad.");
        return { timestamp: now.toISOString(), transitGates, activatedChannels, reinforcedGates, interpretation };
      }),
  }),
